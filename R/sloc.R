...
#' @export
sloc_cloc_engine <- function(binary = "cloc", arguments = c("--follow-links", "-q", "--csv", "--by-file")) {
    result <- function(path, ...) {
        ...
        if (length(result$stdout) > 0) {
            sloc <- read_csv(result$stdout, col_types="cciii")
            sloc <- head(sloc, -1)
...
split_sloc <- function(sloc, path) {
    ...
    ignore <- length(path_split(path)[[1]]) + 1
